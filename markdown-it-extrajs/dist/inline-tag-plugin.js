const inlineTagPlugin = (md, name, options) => {
  ...
  const postProcess = (state) => {
    ...
    for (let i = 0; i < delimiters.length; i++) {
      ...
      tokenClose.content = "";
      if (state.tokens[endDelim.token - 1].type === "text" && loneMarkerRegExp.test(state.tokens[endDelim.token - 1].content)) {
    ...
    while (loneMarkers.length) {
      ...
      let j = i + 1;
      while (j < state.tokens.length && state.tokens[j].type === `inline_tag_${name}_close`) {
        j++;
      }
      j--;
      if (i !== j) {
...
var inline_tag_plugin_default = inlineTagPlugin;
export {
  inline_tag_plugin_default as default
};
//# sourceMappingURL=inline-tag-plugin.js.map
