...
async function convertHtmlFileToPdf(htmlFilePath, outputPdfPath) {
    ...
    await page.setContent(htmlContent, { timeout: 120000, waitUntil: ['load', 'networkidle0'] });

    await page.pdf({
        // format: 'A4', 
        path: outputPdfPath, width: '1280px',
        height: '720px',
        printBackground: true, displayHeaderFooter: false, margin: { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' }
    });
    await browser.close();
}

const htmlFilePath = './dist/sample.html';
const outputPdfPath = './dist/sample_puppeteer.pdf';
